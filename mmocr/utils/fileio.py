...
def list_from_folder_table(folder, max_seq_len):
    ...
    for file in files:
        ...
        with open(file, 'r') as f:
            ...
            cell_content = []
            for line in lines:
                bboxes.append(convert_bbox(line.strip().split(bbox_content_split)[0].strip().split(bbox_split)))
                # if bbox is not empty
                if line.strip().split(bbox_content_split)[0].strip() != '0,0,0,0':
                    cell_content.append(line.strip().split(bbox_content_split)[1].strip().split(cell_split))

            # print_nam
            # if len(cell_content) > 400:
            #     print(file_path)
            # cell_content: list(list) list of cells (list of char in cell)
            item_dict['cell_content'] = cell_content
            # namly
