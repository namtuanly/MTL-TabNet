...
class MultiHeadLocalAttention(nn.Module):
    ...
    def __init__(self, headers, d_model, dropout, window_size=0):
        ...
        self.attn = None
        self.local_attention = LocalAttention(dim=self.d_k, window_size=window_size, causal=True,
...
class DecoderLayerCell(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        ...
        self.size = size
        self.self_attn = MultiHeadGlobalAttention(**self_attn)
...
class TableMasterDecoder(BaseDecoder):
    ...
    def make_mask_cell(self, src, tgt):
        ...
        :return:
        """
        trg_pad_mask = (tgt != self.PAD_CELL).unsqueeze(1).unsqueeze(3).byte()
