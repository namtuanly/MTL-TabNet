...
def build_bbox_mask(label):
    # TODO : namly need to debug to keep <eb></eb> or not.
    # <eb></eb>, <eb1></eb1> ... <eb10></eb10> : empty cell
    structure_token_nums = len(label.split(','))
    pattern = ['<td></td>', '<td', '<eb></eb>']
...
class TableStrParser:
    ...
    def get_item(self, data_ret, index):
        ...
        # namly
        bboxes = line_dict['bbox']   # list(list): list of cell coordinates (list of 4 coordinates)
        cell_content = line_dict['cell_content']  # list(list): list of cell contents (list of char)
        ...
        line_info['cell_content'] = cell_content
