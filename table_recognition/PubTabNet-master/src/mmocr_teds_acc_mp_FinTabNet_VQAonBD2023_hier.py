...
def visual_pred_bboxes(bboxes, filename, dir_path, dir_img):
    ...
    for new_bbox in new_bboxes:
        ...
        if int(new_bbox[0]) == int(new_bbox[2]) and int(new_bbox[1]) == int(new_bbox[3]):
            continue
        new_bboxes_.append((int(new_bbox[0]), int(new_bbox[1]), int(new_bbox[2]), int(new_bbox[3])))
...
if __name__ == "__main__":
    ...
    infer_html_dir = predFile + 'infer_html_dat/'
    ...
    for idx, (file_name, context) in enumerate(predDict.items()):
        ...
        # # visualize bboxes
        new_bboxes_ = visual_pred_bboxes(context['bbox'], file_name, predFile_vis, fintabnet_dir)
        f_bbox = open(os.path.join(infer_html_dir, file_name.replace('.png', '.dat')), 'wb')
        np.save(f_bbox, new_bboxes_)
        f_bbox.close()
