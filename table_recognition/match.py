...
from table_recognition.utils import xywh2xyxy, xyxy2xywh, remove_empty_bboxes
...
def match_visual(file_name, match_list, end2end_xyxy, master_xyxy, prex='ordinary_match'):
    """
    ...
    for idx, match_group in enumerate(match_list):
        ...
        if master_index < master_nums:
            ...
            img_master = cv2.rectangle(img_master, (int(master_bbox[0]), int(master_bbox[1])),
                                       (int(master_bbox[2]), int(master_bbox[3])), bbox_color, thickness=1)
            master_text_coord = (int(master_bbox[0]) - 4, int(master_bbox[1]))
            img_master = cv2.putText(img_master, str(master_index), master_text_coord, 1, 1, text_color, 2)

        # draw end2end
        end2end_bbox = end2end_xyxy[end2end_idx]
        img_end2end = cv2.rectangle(img_end2end, (int(end2end_bbox[0]), int(end2end_bbox[1])),
                            (int(end2end_bbox[2]), int(end2end_bbox[3])), bbox_color, thickness=1)
        end2end_text_coord = (int(end2end_bbox[0])-4, int(end2end_bbox[1]))
        # write end2end bbox matching master bbox's index
        img_end2end = cv2.putText(img_end2end, str(master_index), end2end_text_coord, 1, 1, text_color, 2)
    ...
    for master_token in master_token_list:
        if master_token.startswith('<td'):
            if text_count > len(match_text_dict)-1:
                ...
                master_token = master_token.replace('><', '>{}<'.format(match_text_dict[text_count]))
                text_count += 1
...
class Matcher:
    ...
    def match(self):
        ...
        for idx, (file_name, end2end_result) in enumerate(self.end2end_results.items()):
            print("[{} / {}] Match file : {} .".format(idx+1, len(self.end2end_results), file_name))
            match_list = []
            ...
            if len(centerIou_no_match_master_indexs) > 0 and len(centerIou_no_match_end2end_indexs) > 0:
                centerIou_no_match_end2end_xywh = end2end_xywh_bboxes[centerIou_no_match_end2end_indexs]
                centerIou_no_match_master_xywh = structure_master_xywh_bboxes[centerIou_no_match_master_indexs]
                distance_match_list = distance_rule_match(centerIou_no_match_end2end_indexs,
    ...
    def get_merge_result(self, match_results):
        ...
        for idx, (file_name, match_info) in enumerate(match_results.items()):
            print("[{} / {}] Merge file : {} .".format(idx+1, len(match_results), file_name))
            end2end_info = self.end2end_results[file_name]
...
if __name__ == "__main__":
    save_root = '/disks/vaskar/exodus/icdar-2021-competition/data/table_master_pretrained/work_dirs'
    end2end_file = save_root + '/end2end_val_result/'
    structure_master_file = save_root + '/structure_val_result/'
    ...
    results_save_folder = save_root + '/final_result/'
