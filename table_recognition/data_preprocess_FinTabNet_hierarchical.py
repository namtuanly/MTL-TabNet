...
class PubtabnetParser(object):
    def __init__(self, jsonl_path, is_toy=True, split='val', is_pse_preLabel=False, chunks_nums=16):
        self.split = split
        self.raw_img_root = '/disks/strg16-176/nam/data/fintabnet/img_tables/'
        self.save_root = '/disks/vaskar/nam/data/mmocr_fintabnet_recognition_0726_' + split + '_hierarchical/'
        ...
        self.dataset_size = 64 if is_toy else float("inf")
    ...
    def merge_token(self, token_list):
        ...
        pointer = 0
        merge_token_list = []
        # </table> is the last token str.
        while token_list[pointer] != '</table>':
            if token_list[pointer] == '<td>':
                ...
                pointer += 1
        merge_token_list.append('</table>')
        return merge_token_list

    def make_hierarchical(self, token_list, cells):
        """
        This function used to make the hierarchical information based on bounding boxes of cell content.
        :param token_list: [list]. the raw tokens from the json line file.
        :param cells: [list]. the list of cell
        :return: added tokens.
        """
        hierarchical_flag = False
        hierarchical_td_store = -1
        cell_count = 0
        pointer = 0
        merge_token_list = []
        # </table> is the last token str.
        while token_list[pointer] != '</table>':
            if token_list[pointer] == '<td' or token_list[pointer] == '<td></td>' or token_list[pointer] == '<eb></eb>':
                cell_count += 1
                merge_token_list.append(token_list[pointer])
                pointer += 1
                continue

            # if hierarchical_td_store != -1 and the first cell of row if not spanning column, then check hierarchical
            if hierarchical_td_store != -1 and token_list[pointer] == '<tr>' and token_list[pointer+1] == '<td></td>':
                # check hierarchical
                cell_spanning = list(np.int_(cells[hierarchical_td_store]['bbox']))
                current_cell = list(np.int_(cells[cell_count]['bbox']))
                if current_cell[0] - cell_spanning[0] >= 0:
                    # hierarchical
                    hierarchical_flag = True
                    merge_token_list.append(token_list[pointer])
                    merge_token_list.append('<td1></td1>')
                    pointer += 2
                    cell_count += 1
                else:
                    # not hierarchical
                    hierarchical_td_store = -1
                    merge_token_list.append(token_list[pointer])
                    merge_token_list.append(token_list[pointer+1])
                    pointer += 2
                    cell_count += 1
                continue

            # if the first cell of the row is spanning column, then check hierarchical
            if token_list[pointer] == '<tr>' and token_list[pointer+1] == '<td' and token_list[pointer+2].startswith(' colspan'):
                hierarchical_td_store = cell_count
                merge_token_list.append(token_list[pointer])
                merge_token_list.append(token_list[pointer+1])
                merge_token_list.append(token_list[pointer+2])
                pointer += 3
                cell_count += 1
                continue
        ...
        merge_token_list.append('</table>')
        return merge_token_list, hierarchical_flag
    ...
    def parse_single_chunk(self, this_chunk, chunks_idx):
        ...
        :return:
        """
        print("chunks_idx {}".format(chunks_idx))
        ...
        for item in tqdm(self.data_generator):
            ...
            for idx, cell in enumerate(cells):
                ...
                cell['bbox'] = list(np.int_(cell['bbox']))

                if cell['bbox'][0] > cell['bbox'][2] or cell['bbox'][1] > cell['bbox'][3]:
                    total_errors += 1
                    skip_sample = True
                elif cell['bbox'][2] > img.shape[1] + 1 or cell['bbox'][3] > img.shape[0] + 1:
                    total_errors += 1
                    skip_sample = True
            ...
            # check hierarchical
            encoded_token, is_hierarchical = self.make_hierarchical(encoded_token, cells) # check hierarchical
            if not is_hierarchical:
                os.remove(txt_filepath)
                continue

            original_img_path = self.save_root + 'original_img/'
            ...
            for idx, cell in enumerate(cells):
                if 'bbox' not in cell.keys():
                    bbox_line = ','.join([str(0) for _ in range(4)]) + bbox_content_split + unknown_token + '\n'
                else:
                    bbox_line = ','.join([str(b) for b in cell['bbox']])

                    # add cell content to bbox line
                    content = cell['tokens']

                    text = '\t'.join([t for t in content])
                    bbox_line = bbox_line + bbox_content_split + text + '\n'
            ...
            idx_ += 1
        print("chunks_idx {} number images {} errors {}".format(chunks_idx, idx_, total_errors))
...
if __name__ == '__main__':
    ...
    jsonl_path = r'/disks/strg16-176/nam/data/fintabnet/img_tables/FinTabNet_1.0.0_table_' + split + '.jsonl'

    parser = PubtabnetParser(jsonl_path, is_toy=False, split=split, is_pse_preLabel=False, chunks_nums=nproc)

    # multiprocessing
    start_time = time.time()
    filenames, count = parser.get_filenames(split=split)
    ...
    print("parse {} images.".format(count))
