    ...
    for master_token in master_token_list:
        if master_token.startswith('<td'):
            if text_count > len(cell_content_list)-1:
                ...
                master_token = master_token.replace('><', '>{}<'.format(BeautifulSoup(cell_content_list[text_count], "lxml").text))
                text_count += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tablemaster_config', type=str,
                        default='./configs/textrecog/master/table_master_new_decoder_WikiTabNet_ja_full_v3_cell100_batch4.py',
    ...
    save_visual_dir = root_dir + '/visual_pred_bboxes/' + args.checkpoint + '/'
    ...
    for subdir, dirs, files in os.walk(root_dir + 'table_img/'):
        for file_ in files:
            ...
            if filepath.endswith(".png"):
                ...
                tablemaster_result, tablemaster_result_dict = tablemaster_inference.predict_single_file(filepath)

                pred_text = tablemaster_result_dict[file_]['text']
                pred_cells = tablemaster_result_dict[file_]['cell']
                ...
                # # visualize bboxes
                visual_pred_bboxes(tablemaster_result_dict[file_]['bbox'], file_, save_visual_dir, root_dir + 'table_img/')

                f = open(os.path.join(save_visual_dir, file_.replace('.png', '.txt')), 'w')
                f.write(pred_html + '\n')
                f.close()

                # table_ = BeautifulSoup(pred_html, "lxml")

                table_MN = pd.read_html(pred_html)
                table_MN[0].to_csv(os.path.join(save_visual_dir, file_.replace('.png', '.csv')))
                print(pred_html)
